...
# Path secret from PLAID_WEBHOOK_SECRET in /etc/budget-bot.env (generated on first serve).
# Do not change the mount prefix: Plaid dashboard already has /plaid-wh-<secret>.
set -euo pipefail
if [ -n "${BUDGET_BOT_ENV:-${HERMES_PLAID_ENV:-}}" ]; then
  ENV_FILE="${BUDGET_BOT_ENV:-$HERMES_PLAID_ENV}"
elif [ -f /etc/budget-bot.env ]; then
