# agent-jobs

Thin scheduled-agent runner for zaz-astra (**irritation #4 fix**: one convention).

## Convention

| Piece | Path |
|-------|------|
| Runner | `scripts/agent-run.sh <job-id> [--force]` |
| Job body | `agent-jobs/<job-id>.sh` (executable) |
| Disable | `~/.config/agent-jobs/<job-id>.disabled` |
| Lock / log / last | `~/.local/state/agent-jobs/` |
| Timer | oneshot user service → `agent-run.sh <id>` |

Optional: `AGENT_TIMEOUT_SECS` (default 300), `AGENT_EMAIL_ON_FAIL=1` for generic fail mail.

## Jobs

| Job | Timer | Purpose |
|-----|-------|---------|
| `ops-log` | 6h | Machine JSON box snapshot |
| `git-access-check` | daily 12:30 PT | PAT can see+push expected repos |
| `consumer-health` | daily 12:45 PT | Smoke + Grok Build path; Claude journal secrets must stay purged |
| `pause-reminder` | Sun 17:00 PT | Grok journal autopilot still paused? |
| `grok-journal` | **02:00 PT** (`grok-journal.timer` → `scripts/grok-journal-run.sh`) | Nightly Grok journal via xAI API (needs `~/.config/grok-journal/enabled`) |
| `budget-bot-watch` | *(no timer yet)* | Budget hardcap/pace/anomaly; fixture dry-run until PayPal Link |
| `budget-bot-eom-leftover` | **1st 09:00 PT** (`budget-bot-eom-leftover.timer`) | Prior-month leftover = calendar STS; Pushover congrats if > $0 |
| `or-timeout-review` | **1st + 15th 10:15 PT** | Compare panel/oracle ungrounded-OR timeout rates vs 2026-08-09 A+B baseline; pure stats; **local log only** (no email/Pushover). First look **done 2026-09-08**. |

Budget Bot code lives in `astra-config/budget-bot` (live path `/root/budget-bot`). Job invokes `python3 -m budget_bot watch`. Live notify: `BUDGET_BOT_LIVE=1` or `notify_enabled` in state config. See `/root/budget-bot/README.md`.

## New private GitHub repo checklist

1. Create private repo  
2. **Add it to the fine-grained PAT** allowlist used in `~/.git-credentials` (Contents R/W)  
3. Add `owner/repo` to `~/.config/agent-jobs/git-repos.list` if not in defaults  
4. `agent-run.sh git-access-check --force` should go green  
5. Clone under `/root/…` and prove `git push`
